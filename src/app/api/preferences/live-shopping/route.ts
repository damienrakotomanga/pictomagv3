...
import {
  readLiveShoppingPreferencesServer,
  seedPreferencesStoreIfMissing,
  writeLiveShoppingPreferencesServer,
...
export async function GET(request: NextRequest) {
  ...
  const preferences = await readLiveShoppingPreferencesServer(resolvedUser.userId);
...
export async function PUT(request: NextRequest) {
  ...
  const preferences = await writeLiveShoppingPreferencesServer(payload?.preferences, resolvedUser.userId);
  const response = NextResponse.json({
    preferences,
    userId: resolvedUser.userId,
  });
